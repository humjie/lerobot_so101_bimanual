...
class OpenCVCamera(Camera):
    ...
    def _validate_fourcc(self) -> None:
        ...
        if actual_fourcc != self.config.fourcc:
            logger.warning(
                f"{self} failed to set fourcc={self.config.fourcc} (actual={actual_fourcc}). "
    ...
    def async_read(self, timeout_ms: float = 20000) -> NDArray[Any]:
        """
        ...
        Args:
            timeout_ms (float): Maximum time in milliseconds to wait for a frame
                to become available. Defaults to 20000ms (20 seconds).
    ...
    def disconnect(self) -> None:
        ...
        if self.videocapture is not None:
            try:
                ...
                logger.warning(f"{self} error during release: {e}")
            finally:
                self.videocapture = None
